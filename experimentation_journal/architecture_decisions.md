...
## 2️⃣1️⃣ Anti-Collapse VQ Mechanisms (Focal Loss & Surgical Restart) – *Why?*
- **Change**: Upgraded `PatchDecoder` to utilize Foreground-Weighted Focal Loss instead of standard Cross-Entropy. Augmented `FactorizedVectorQuantizer` with usage-tracking buffers and `resurrect_dead_codes()`.
- **Rationale**: An analysis of W&B charts revealed a catastrophic "Background Collapse." The model achieved low reconstruction loss by entirely ignoring colored blocks and predicting "all-black" patches, which only required 4 codebook shape entries, effectively killing the other 252. The Focal Loss mathematically ignores the easy background and scales up the penalty for ignoring colored shapes by $50\times$. Because unselected codes receive zero gradients, `resurrect_dead_codes()` performs surgical memory resets by overwriting unused codes with highly-active live patches from the current batch.
- **Expected Behaviour**: 100% dictionary code utilization. A permanent end to Index Starvation, with the model successfully learning hundreds of complex color/shape combinations instead of lazy background patches.

## 2️⃣2️⃣ Padding-Masked Perplexity & Surgery Pool – *Why?*
